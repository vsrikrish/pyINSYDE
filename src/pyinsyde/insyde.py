...
class Building:
    ...
    def __init__(self, building_data:BuildingProperties):
        ...
        if self.rv_path is None:
            rep_val_data = read_replacement_values(join(dirname(abspath(__file__)), "data", "replacement_values.txt"))
        else:
        ...
        if self.up_path is None:
            unit_cost_data = read_unit_prices(join(dirname(abspath(__file__)), "data", "unit_prices.txt"))
        else:
